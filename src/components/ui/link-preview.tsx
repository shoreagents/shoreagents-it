...
export const LinkPreview = ({
  ...
  if (!isStatic) {
    ...
    usp.set("viewport.isMobile", "true");
    usp.set("viewport.deviceScaleFactor", "1");
    usp.set("viewport.width", String(width * 3));
    usp.set("viewport.height", String(height * 3));
    src = `https://api.microlink.io/?${usp.toString()}`;
  } else {
    src = imageSrc;
  }
  ...
  return (
    <>
      ...
      <HoverCardPrimitive.Root openDelay={50} closeDelay={100} onOpenChange={setOpen}>
        <HoverCardPrimitive.Trigger onMouseMove={handleMouseMove} className={cn("text-black dark:text-white cursor-pointer", className)}>
          {children}
