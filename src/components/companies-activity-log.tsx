...
export function CompaniesActivityLog({ companyId, companyName, onRefresh }: ActivityLogProps) {
  ...
  const fetchActivitiesAndComments = useCallback(async (page: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/companies/${companyId}/activity?page=${page}&limit=20`)
  ...
  // Initial load
  useEffect(() => {
    console.log('🚀 Initial load useEffect triggered for companyId:', companyId)
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/companies/${companyId}/activity?page=1&limit=20`)
    ...
    loadInitialData()
  }, [companyId]) // Only depend on companyId, not on fetchActivitiesAndComments
