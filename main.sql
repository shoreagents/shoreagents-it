...
-- Table Triggers for talent_pool
create trigger update_talent_pool_updated_at before
update on public.talent_pool for each row execute function update_updated_at_column();

create trigger trigger_talent_pool_changes after
insert or delete or update on public.talent_pool for each row execute function notify_talent_pool_changes();
...
-- DROP FUNCTION public.notify_talent_pool_changes();

CREATE OR REPLACE FUNCTION public.notify_talent_pool_changes()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- Send notification
    PERFORM pg_notify('talent_pool_changes', payload::text);
